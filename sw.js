...
const CACHE = 'kntt-v3101-main-screen-fit';
const SERVED_GAME_VERSION = '3.10.1';
...
/* kntt-v3101: single-screen menu fit + unified UI foundation */
(function(){
  if (window.__KNTT_PATCH_V3101__) return;
  ...
  window.__KNTT_PATCH_V3101__ = true;
  ...
  function defaultProgress(profile){
    return { talents:{d:0,s:0,h:0,c:0}, unitSkills:{}, tp:0, best:[0,0,0], sfx:(typeof Sound === 'undefined') ? true : Sound.sfxOn !== false, music:(typeof Sound === 'undefined') ? false : !!Sound.musicOn, gems:0, stageProg:0, unlocked:[], profile:profile || {name:'Tân Thủ',avatar:'🐱',level:1,xp:0,totalXp:0,createdAt:new Date().toISOString(),configured:true} };
  }
  ...
  window.calcEnemyXp = function(e){
    try { const base = Math.max(1,(e && e.db && e.db.xp)||1), dbHp = Math.max(1,(e && e.db && e.db.hp)||1), hpMul = Math.max(1,((e && e.maxHp)||dbHp)/dbHp), stageDepth = 1 + Math.max(0, State.stageIdx || 0) * 0.035, typeMul = (e && e.db && e.db.isBoss) ? 4.2 : ((e && e.db && e.db.isHeavy) ? 1.45 : 1); return Math.max(1, Math.round(base * stageDepth * typeMul * Math.pow(hpMul, 0.52))); } catch(_) { return 1; }
  };
  ...
  function installCss(){
    if (q('ui-v3101-css')) return;
    const st = document.createElement('style'); st.id = 'ui-v3101-css';
    st.textContent =
      '#start{overflow:hidden!important;padding:6px!important;align-items:center!important;justify-content:center!important}' +
      '#s-card{height:calc(100% - 12px)!important;max-height:none!important;display:flex!important;flex-direction:column!important;margin:auto!important}' +
      '.s-hero{flex:0 0 62px!important;padding:6px 12px!important}.s-hero-title{font-size:clamp(21px,4vw,32px)!important;line-height:1.05!important}.s-hero-sub{font-size:10px!important;margin-top:3px!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important}' +
      '.s-body{flex:1!important;min-height:0!important;padding:8px 12px 10px!important;gap:10px!important}.s-col-main{display:flex!important;flex-direction:column!important;min-height:0!important}.s-col-main>div:first-child{flex:0 0 30px!important;margin-bottom:5px!important}.s-sec-title{font-size:18px!important}' +
  ...
  function openTalentsV3101(){
    installCss(); if (!UI._talTab) UI._talTab = 'common';
    ...
    if (!content) return;
    const tabs = q('tmod').querySelectorAll('.ui-tab'); tabs[0].onclick = function(){ UI._talTab='common'; Sound.play('click'); openTalentsV3101(); }; tabs[1].onclick = function(){ UI._talTab='units'; Sound.play('click'); openTalentsV3101(); };
    if (UI._talTab === 'common') {
      Object.keys(TALENTS_DB).forEach(function(k){ const t=TALENTS_DB[k], lv=State.talents[k]||0, canUp=State.tp>0 && lv<t.max; const row=document.createElement('div'); row.className='ui-row'; row.innerHTML='<div class="ui-ico">'+t.icon+'</div><div class="ui-main"><div class="ui-name">'+esc(t.name)+'<span class="ui-lv">Lv.'+lv+'/'+t.max+'</span></div><div class="ui-sub">'+esc(t.desc)+'</div></div><button class="ui-act '+(canUp?'':'off')+'">'+(lv>=t.max?'MAX':'+')+'</button>'; if(canUp) row.querySelector('button').onclick=function(){ State.tp--; State.talents[k]++; if(k==='h') State.hp+=100; Sound.play('upgrade'); Storage.sync(); UI.updateDisplay(); UI.refreshStart(); openTalentsV3101(); }; content.appendChild(row); });
    } else {
      Object.keys(UNIT_SKILLS).forEach(function(k){ const u=UNITS_DB[k], tiers=UNIT_SKILLS[k], lv=State.unitSkills[k]||0, next=tiers[lv], owned=isUnlocked(k), canUp=owned && next && State.tp>=next.cost; const row=document.createElement('div'); row.className='ui-row'; if(!owned) row.style.opacity='.55'; const info=!owned?'Chưa mở tướng. Mở trong Cửa hàng trước.':(next?('Bậc '+(lv+1)+': '+next.n+' — '+next.d+(lv===2?' · Mở Hoá Thần cấp 4':'')):'Đã mở toàn bộ kỹ năng · Có thể Hoá Thần cấp 4 trong trận'); const btn=!owned?'🔒':(!next?'MAX':(next.cost+' SP')); row.innerHTML='<div class="ui-ico">'+u.icon+'</div><div class="ui-main"><div class="ui-name" style="color:'+u.color+'">'+esc(u.name)+'<span class="ui-lv">Lv.'+lv+'/'+tiers.length+'</span></div><div class="ui-sub">'+esc(info)+'</div></div><button class="ui-act '+(canUp?'':'off')+'">'+btn+'</button>'; if(canUp) row.querySelector('button').onclick=function(){ State.tp-=next.cost; State.unitSkills[k]=lv+1; Sound.play('upgrade'); Storage.sync(); UI.updateDisplay(); UI.refreshStart(); openTalentsV3101(); }; content.appendChild(row); });
    }
    q('tmod').classList.remove('hidden');
  }

  function openShopV3101(){
    installCss(); if (!UI._shopTab) UI._shopTab = 'all';
    ...
    if (!content) return;
    const tabs = q('hmod').querySelectorAll('.ui-tab'); tabs[0].onclick=function(){UI._shopTab='all';Sound.play('click');openShopV3101();}; tabs[1].onclick=function(){UI._shopTab='owned';Sound.play('click');openShopV3101();}; tabs[2].onclick=function(){UI._shopTab='locked';Sound.play('click');openShopV3101();};
    Object.keys(UNITS_DB).forEach(function(k){ const u=UNITS_DB[k], cost=HERO_COST[k], owned=isUnlocked(k), free=HERO_COST[k]===0; if(UI._shopTab==='owned'&&!owned) return; if(UI._shopTab==='locked'&&owned) return; const canBuy=!owned && Storage.data.gems>=cost; const row=document.createElement('div'); row.className='ui-row'; const right=owned?'<span class="ui-badge">'+(free?'Mặc định':'Đã mở')+'</span>':'<button class="ui-act '+(canBuy?'':'off')+'">'+cost+'💎</button>'; row.innerHTML='<div class="ui-ico">'+u.icon+'</div><div class="ui-main"><div class="ui-name" style="color:'+u.color+'">'+esc(u.name)+'</div><div class="ui-sub">'+esc(u.lv3Desc||'Tướng chiến đấu')+'</div></div>'+right; if(!owned&&canBuy) row.querySelector('button').onclick=function(){ Storage.data.gems-=cost; Storage.data.unlocked=Storage.data.unlocked||[]; if(!Storage.data.unlocked.includes(k)) Storage.data.unlocked.push(k); Storage.sync(); Sound.play('upgrade'); vibe(20); UI.refreshStart(); openShopV3101(); }; content.appendChild(row); });
    q('hmod').classList.remove('hidden');
  }

  function refreshUpgradeButton(){ try { const u=State.ui&&State.ui.selUnit, modal=q('umod'), btnUp=q('b-up'); if(!u||!modal||modal.classList.contains('hidden')||!btnUp) return; const maxLv=maxUnitLevel(u.typeId); if(u.level>=maxLv){btnUp.style.display='none';return;} const cost=upgradeCost(u), canUp=State.gold>=cost; btnUp.style.display='flex'; const costEl=q('um-c'); if(costEl) costEl.innerText='🪙 '+cost+(u.level===3?' · Hoá Thần':''); btnUp.className=canUp?'flex-1 bg-gradient-to-b from-blue-500 to-blue-700 hover:from-blue-400 rounded py-1 flex flex-col items-center border border-blue-400 shadow-sm':'flex-1 bg-slate-700 rounded py-1 flex flex-col items-center opacity-50 cursor-not-allowed'; } catch(_){} }

  function ensureProfileUi(){
    installCss();
    let card=q('profile-card'); const side=document.querySelector('.s-col-side'), startBtn=q('b-start');
    if(!card&&side&&startBtn){ card=document.createElement('div'); card.id='profile-card'; card.innerHTML='<div class="p-row"><div class="p-av" id="prof-av">🐱</div><div style="min-width:0;flex:1"><div class="p-name" id="prof-name">Tân Thủ</div><div class="p-sub"><span id="prof-lv">Lv.1</span> · <span id="prof-xptxt">0/0 EXP</span></div></div></div><div class="p-xp"><div id="prof-xpbar"></div></div><button class="p-btn" id="b-profile">👤 Hồ sơ</button>'; side.insertBefore(card,startBtn); card.querySelector('#b-profile').onclick=openProfileModal; card.querySelector('#prof-av').onclick=openProfileModal; }
    let modal=q('prof-modal');
    if(!modal){ modal=document.createElement('div'); modal.id='prof-modal'; modal.innerHTML='<div id="prof-box"><h2 class="title title--grad" style="font-size:24px;text-align:center;margin-bottom:8px">Hồ sơ nhân vật</h2><p class="muted" style="text-align:center;font-size:11px;margin-bottom:12px">Tạo hồ sơ mới sẽ reset tiến trình để chơi lại từ đầu.</p><input id="prof-name-input" maxlength="18" placeholder="Tên nhân vật"><div id="prof-avatars"></div><div class="prof-actions"><button class="prof-close" id="prof-cancel">Đóng</button><button class="prof-save" id="prof-save">Lưu</button><button class="prof-reset" id="prof-reset">Tạo hồ sơ mới / Chơi lại từ đầu</button></div></div>'; document.body.appendChild(modal); modal.querySelector('#prof-cancel').onclick=function(){modal.classList.remove('show');}; modal.querySelector('#prof-save').onclick=saveProfileModal; modal.querySelector('#prof-reset').onclick=resetFromProfileModal; const grid=modal.querySelector('#prof-avatars'); AVATARS.forEach(function(a){const b=document.createElement('button'); b.textContent=a; b.onclick=function(){selectedAvatar=a; renderAvatarChoices();}; grid.appendChild(b);}); }
  }
  ...
  const _refreshStart=UI.refreshStart&&UI.refreshStart.bind(UI); UI.refreshStart=function(){ syncStateFromProfile(); const r=_refreshStart?_refreshStart():undefined; renderProfileCard(); return r; };
  const _updateDisplay=UI.updateDisplay&&UI.updateDisplay.bind(UI); UI.updateDisplay=function(){ syncStateFromProfile(); const r=_updateDisplay?_updateDisplay():undefined; refreshUpgradeButton(); renderProfileCard(); return r; };
  UI.openTalents = openTalentsV3101; UI.openShop = openShopV3101;

  const _draw=Engine.draw&&Engine.draw.bind(Engine); Engine.draw=function(){ try{ if(State.particles&&State.particles.length>120) State.particles.splice(0,State.particles.length-120); if(State.floatTexts&&State.floatTexts.length>28) State.floatTexts.splice(0,State.floatTexts.length-28); if(State.flyingCoins&&State.flyingCoins.length>18) State.flyingCoins.splice(0,State.flyingCoins.length-18); if(State.beams&&State.beams.length>18) State.beams.splice(0,State.beams.length-18); }catch(_){} return _draw?_draw():undefined; };
  const _initMap=Engine.initMap&&Engine.initMap.bind(Engine); if(_initMap) Engine.initMap=function(){ const r=_initMap(); if(State.weather&&State.weather.length>18) State.weather.length=18; return r; };
  ...
  const _stageWin=Control.stageWin&&Control.stageWin.bind(Control); Control.stageWin=function(){ try{ State.running=false; const first=State.stageIdx>=Storage.data.stageProg; const reward=stageGemReward(State.stage,first); const xp=stageClearXp(State.stage,first); Storage.data.gems+=reward; if(State.stageIdx+1>Storage.data.stageProg) Storage.data.stageProg=State.stageIdx+1; Engine.gainXp(xp); directSync(); UI.refreshStart(); Sound.play('cleared'); vibe([0,60,40,120]); q('start').classList.remove('hidden'); q('g-stat').classList.remove('hidden'); q('g-stat').querySelector('.title').innerText=first?'🎉 Qua màn lần đầu!':'Qua màn · Cày lại'; q('g-wav').innerHTML=esc(State.stage.n)+'<div style="font-size:13px;color:var(--gold);margin-top:5px">+'+reward+' 💎 · +'+xp+' EXP</div>'; q('b-start').innerText='CHƠI TIẾP'; q('pmod').classList.remove('show'); UI.selStage=Math.min(Storage.data.stageProg,STAGES.length-1); }catch(e){ if(_stageWin) return _stageWin(); } };
  const _gameOver=Control.gameOver&&Control.gameOver.bind(Control); Control.gameOver=function(){ try{ State.running=false; directSync(); UI.refreshStart(); Sound.play('lose'); vibe([0,100,50,100,50,200]); q('start').classList.remove('hidden'); q('g-stat').classList.remove('hidden'); q('g-stat').querySelector('.title').innerText='Thất bại tại'; q('g-wav').innerHTML=esc(State.stage.n)+'<div style="font-size:12px;color:var(--dim);margin-top:4px">Đợt '+State.wave+'/'+State.stage.waves+' · EXP đã cày được vẫn giữ</div>'; q('b-start').innerText='CHƠI LẠI MÀN'; q('pmod').classList.remove('show'); }catch(e){ if(_gameOver) return _gameOver(); } };
...
function patchIndexText(text) {
  let out = text
    ...
    .replace("const cont = document.getElementById('canv-cont'); const dpr = window.devicePixelRatio || 1;", "const cont = document.getElementById('canv-cont'); const dpr = Math.min(window.devicePixelRatio || 1, 1.35);")
    .replace("State.running = true; State.paused = false; State.speed = 1; document.getElementById('b-speed').innerText = '1x'; document.getElementById('b-speed').classList.remove('on');", "State.running = true; State.paused = false; State.speed = 0.9; document.getElementById('b-speed').innerText = '0.9x'; document.getElementById('b-speed').classList.remove('on');")
    .replace("State.level = 1; State.xp = 0; State.maxXp = CFG.xpBase;", "State.level = Math.max(1, (Storage.data.profile && Storage.data.profile.level) || 1); State.xp = Math.max(0, (Storage.data.profile && Storage.data.profile.xp) || 0); State.maxXp = playerXpNeed(State.level);")
    .replace("if (State.running && !State.paused) { for (let i = 0; i < State.speed; i++) Engine.update(raw); }", "if (State.running && !State.paused) { Engine.update(raw * (State.speed || 1)); }")
    .replace("let reward = State.stage.reward * (first ? 1 : 0.3);", "let reward = stageGemReward(State.stage, first);")
    .replace("Storage.data.gems += reward;", "Storage.data.gems += reward; Engine.gainXp(stageClearXp(State.stage, first));")
    .replace("this.gainXp(e.db.xp);", "this.gainXp(calcEnemyXp(e));")
    .replace("UI.showMessage(WAVE_THEMES[waveTheme(State.wave)].hint, waveTheme(State.wave) === 'boss'); Sound.play('wave'); this.buildWave();", "UI.showMessage(WAVE_THEMES[_th].hint, _th === 'boss'); Sound.play('wave'); this.buildWave();");
  if (!out.includes('__KNTT_PATCH_V3101__')) out = out.replace('// ============ BOOT ============', `${ALL_PATCH_SCRIPT}\n// ============ BOOT ============`);
  return out;
}
